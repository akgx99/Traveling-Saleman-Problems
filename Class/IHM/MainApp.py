...
class MainApp(tk.Frame):
    def __init__(self, master=None):
        ...
        self._buttonAlgo1 = tk.Button(self, text="Tournée Croissante",
                                      command=lambda name="IncreasingTour": self.LaunchAlgorithm(name), width=15,
                                      height=1, bg="dodger blue", fg="light cyan", activebackground="lightgray")
        self._buttonAlgo1.pack(side="bottom", pady="10")

        # boutton algo 2 : Tournéé random
        self._buttonAlgo2 = tk.Button(self, text="Au hasard",
                                      command=lambda name="RandomTour": self.LaunchAlgorithm(name), width=15, height=1,
                                      bg="dodger blue", fg="light cyan", activebackground="lightgray")
        self._buttonAlgo2.pack(side="bottom", pady="10")

        # boutton algo 3 : proche voisin
        self._buttonAlgo3 = tk.Button(self, text="Proche voisin",
                                      command=lambda name="closeNeighbor": self.LaunchAlgorithm(name), width=15,
                                      height=1, bg="dodger blue", fg="light cyan", activebackground="lightgray")
    ...
    def openFile(self):
        ...
        self.filePath = askopenfilename(filetypes=(("TSP File", "*.tsp"), ("TSP File", "*.TSP")),
                                        title="Choose a TSP file")  # ouvre une fenêtre de choix de fichier .tsp ou .TSP
        self.clearList()
    ...
    def process(self, file, algo):
        ...
        if len(self.manager.cities) > 0:  # si plus d'une ville existe
            self.algo = Algo.Algo(self.manager.cities,
                                  self.manager.cities[0])  # définit les villes comme sommets et un sommet de départ

            if self._chosenAlgo is not None:
    ...
    def viewCosts(self):
        cost = Calc.costTour(self.algo.visited, self.algo.start)
        self._displayCosts['text'] = str(cost[0]) + " km parcouru"
        print(cost[1])
        print(cost[0])
